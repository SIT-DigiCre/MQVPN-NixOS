#!/usr/bin/env bash
# 起動直後DOWN (iperf -R) 0/0問題の層別トレース。up.sh直後に実行:
...
echo "=== before ==="
snap /tmp/trace-before.txt
cat /tmp/trace-before.txt
echo ""
...
echo ""
echo "=== after ==="
snap /tmp/trace-after.txt
cat /tmp/trace-after.txt

...
### パッチ単独適用の切り分け (scan → rate-limit の差分法)

- **rate-limit 単独は適用不能** (scan パッチの追加行をコンテキストに含むため
  Hunk#2 が失敗)。効果は (scan+rate-limit) − (scan) の差分で評価。
...
- **rate-limit (スキャン間引き) が CPU 飽和を打破する主役**: 800M で CPU 103→58、
  上限 1.1G→1.7G。**scan (1 回あたりコスト削減) は膝効率の改善** (同飽和下で 12%→6.4%
  ロス減、上限小幅 UP)。
- 以下、本文の計測は scan+rate-limit 適用版の数値。
